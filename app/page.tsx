...
export default function PokerApp() {
  ...
  const applyLoansToScore = () => {
    ...
    loans.forEach(loan => {
      if (loan.from !== '在庫') {
        newPoints[loan.from] = getFinalAmount(loan.from) + loan.amount;
        newInputModes[loan.from] = 'yen';
      }
      if (loan.to !== '在庫') {
        newPoints[loan.to] = getFinalAmount(loan.to) - loan.amount;
  ...
  const saveEvent = async () => {
    if (currentTotalAmount !== 0) return alert("合計を0円にしてください");
    ...
    if (error) alert("失敗");
    else { alert("保存成功"); fetchData(); setSelectedIds([]); setPoints({}); setLoans([]); }
  };
  ...
  return (
    <div className="max-w-md mx-auto p-4 bg-slate-50 min-h-screen text-slate-900">
      ...
      {activeTab === 'input' && (
        <>
          <div className="bg-amber-50 p-4 rounded-2xl mb-6 border border-amber-100 text-slate-900">
            <h2 className="text-[10px] font-black text-amber-600 uppercase mb-3 text-slate-900">🤝 貸借メモ</h2>
            ...
            <div className="flex gap-2 mb-3">
              <input type="number" placeholder="金額" value={loanAmount || ""} onChange={(e)=>setLoanAmount(parseInt(e.target.value)||0)} className="flex-1 p-2 text-xs rounded-lg border-none" />
              ...
              <div className="space-y-1">
                {loans.map((l, i) => (
                  <div key={i} className="text-[10px] font-bold text-amber-700 flex justify-between bg-white/50 p-1 px-2 rounded">
                    <span>{l.from} → {l.to}</span><span>{l.amount.toLocaleString()}円</span>
                  </div>
                ))}
          ...
          <div className="bg-white p-5 rounded-2xl shadow-sm mb-6 border border-slate-100 text-slate-900">
            <h2 className="text-xs font-black text-slate-400 uppercase mb-4 text-slate-900">新規セッション</h2>
            ...
            {selectedIds.map(name => (
              <div key={name} className="flex flex-col mb-4 pb-4 border-b border-slate-50 last:border-0 text-slate-900">
                ...
                {(inputModes[name] || 'pt') === 'pt' && <div className="text-[10px] text-right text-slate-400 font-bold mt-1">金額: {(points[name] || 0) / 2} 円</div>}
              </div>
            ))}
            {selectedIds.length > 0 && (
              <div className={`mt-4 p-2 rounded-lg text-center font-bold text-xs ${currentTotalAmount === 0 ? 'text-emerald-600 bg-emerald-50' : 'text-rose-500 bg-rose-50'}`}>
                {currentTotalAmount === 0 ? '✓ 合計0円' : `合計0円にしてください (${currentTotalAmount.toLocaleString()}円)`}
              </div>
            )}
            <button onClick={saveEvent} disabled={selectedIds.length === 0} className="w-full bg-slate-900 text-white py-4 rounded-xl font-black mt-4 disabled:bg-slate-200">保存</button>
          </div>
              ...
              <div className="bg-white w-full max-w-sm rounded-3xl p-6 shadow-2xl">
                ...
                <div className="mb-6 p-4 bg-slate-50 rounded-2xl border border-slate-100">
                  <div className="flex justify-between items-center mb-2">
                    ...
                    <span className="text-xs font-mono font-bold">{initialStack.toLocaleString()} pt</span>
                  </div>
                ...
                <div className="space-y-3 mb-6">
                  {Object.keys(currentChipCounts).map(val => (
                    <div key={val} className="flex items-center justify-between bg-white p-2 rounded-xl border border-slate-100 shadow-sm text-slate-900">
                      <div className={`w-8 h-8 rounded-full border-2 border-dashed flex items-center justify-center text-[10px] font-black`}>{val}</div>
                      <div className="flex items-center gap-2">
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center px-1">
              ...
              <button onClick={() => setFilterUnpaid(!filterUnpaid)} className={`text-[10px] font-black px-3 py-1 rounded-full border ${filterUnpaid ? 'bg-rose-500 text-white' : 'bg-white text-slate-400'}`}>
                {filterUnpaid ? '未清算のみ' : 'すべて表示'}
              </button>
            </div>
      ...
      {activeTab === 'ranking' && (
        <div className="space-y-4">
          <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100 space-y-3">
            <h2 className="text-xs font-black text-slate-400 uppercase">期間指定</h2>
            <div className="flex items-center gap-2 text-xs font-bold">
              <input type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} className="flex-1 p-2 bg-slate-50 rounded-lg outline-none" />
              <span>〜</span>
              <input type="date" value={endDate} onChange={(e) => setEndDate(e.target.value)} className="flex-1 p-2 bg-slate-50 rounded-lg outline-none" />
            </div>
          </div>
      ...
      {activeTab === 'master' && (
        <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100 text-slate-900">
          <h2 className="text-xs font-black text-slate-400 uppercase mb-4 text-slate-900">名簿管理</h2>
          ...
          <div className="space-y-2">
            {members.map(m => (
              <div key={m} className="flex justify-between items-center p-3 bg-slate-50 rounded-lg border border-slate-100 text-slate-900">
