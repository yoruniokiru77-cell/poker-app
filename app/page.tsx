...
import { supabase } from '../app/lib/supabase'; // パスを直書きした環境に合わせて修正
...
export default function PokerApp() {
  ...
  const deleteMember = async (name: string) => {
    if (!confirm(`${name}さんを削除しますか？これまでの成績データからは名前のみ消えます。`)) return;
            ...
            {selectedIds.map(name => (
              <div key={name} className="flex items-center justify-between mb-3">
                ...
                <input type="number" placeholder="0" value={amounts[name] || ""} onChange={(e) => setAmounts({ ...amounts, [name]: parseInt(e.target.value) || 0 })}
                  className="w-28 p-2 border-2 border-slate-100 rounded-lg text-right focus:border-indigo-400 outline-none font-mono text-slate-900" />
              </div>
            ))}
          ...
          <div className="space-y-4">
            ...
            {events.map(ev => (
              <div key={ev.id} className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100 relative">
                <button onClick={() => deleteEvent(ev.id)} className="absolute top-4 right-4 text-slate-300 hover:text-rose-500 transition-colors">
          ...
          <div className="space-y-2">
            {members.map(m => (
              <div key={m} className="flex justify-between items-center p-3 bg-slate-50 rounded-lg border border-slate-100">
                ...
                <button onClick={() => deleteMember(m)} className="text-slate-300 hover:text-rose-500 transition-colors p-1">
